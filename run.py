...
# resolved bug, add condition to prevent an enemy shot when all enemy ships are destroyed, add screenshot of code
...
def game_screen():
    ...
    while torpedo_count > 0 and hull_plates_remaining > 0 and int(len(enemy_ship_locations)) > 0 and int(len(merchant_ship_locations)) > 0 and torpedo_count >= int(len(enemy_ship_locations)):
        ...
        ### 8. ELSE IT IS A MISS ###
            ...
            if battle_grid[int(user_x_coord)][int(user_y_coord)] != '\x1b[93mM\x1b[0m' or battle_grid[int(user_x_coord)][int(user_y_coord)] != '\x1b[92mE\x1b[0m':
